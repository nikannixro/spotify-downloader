...
def _get_downloader(output_dir: str):
    """Create a spotDL Downloader with minimal settings."""
    ...
    return Downloader(
        settings={
            ...
            "overwrite": "force",
            "cookie_file": cfg.COOKIE_FILE or None,
            "search_query": "{title} - {artist} {album}",
            "filter_results": True,
            "yt_dlp_args": None,
            "detect_formats": None,
            "sponsor_block": False,
...
def _run_spotdl_download(downloader, song):
    """Run spotDL download using spotDL's own event loop in the worker thread."""
    asyncio.set_event_loop(downloader.loop)
    return downloader.download_song(song)
...
class SpotDLBackend:
    ...
    async def download_track(self, track_url: str, user_id: int = 0) -> DownloadResult:
        ...
        try:
            ...
            downloader = _get_downloader(tmp_dir)
            result = await loop.run_in_executor(
                None, _run_spotdl_download, downloader, song
            )

            dl_song, file_path = result
            if file_path is None or not file_path.exists():
                return DownloadResult(success=False, error="Download failed")

            meta = _song_to_metadata(dl_song)
            ...
            logger.exception("spotDL download failed for %s: %s", track_url, exc)
    ...
    async def download_song(self, song, user_id: int = 0) -> DownloadResult:
        ...
        try:
            ...
            logger.exception("spotDL download failed: %s", exc)
            return DownloadResult(success=False, error=f"Download error: {exc}")
        finally:
            if user_id:
                unregister_temp_dir(user_id, tmp_dir)
